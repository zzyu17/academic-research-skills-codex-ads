...
此 ADS 版為天文學與天體物理學加入 SAO/NASA ADS 和 arXiv 文獻探索，以及以 bibcode 為基礎的第五路引用驗證。設定 `ADS_API_TOKEN` 可啟用結構化 ADS 存取；未設定時會略過 ADS，並繼續使用其他索引。標準 Codex 版請見 [academic-research-skills-codex](https://github.com/Imbad0202/academic-research-skills-codex)。
...
本倉庫是 ARS-Codex-ADS。如需原始 Claude Code ARS 發行版本，
...
此 ARS-Codex-ADS 套件版本為 `0.1.21`。倉庫根目錄的 `VERSION` 檔案、
...
## 安裝 ARS-Codex-ADS Plugin

透過 Codex CLI 加入 GitHub marketplace 並安裝 ARS-Codex-ADS：
...
`academic-research-suite-ads` 的實體副本而非符號連結，確保 Windows 上的 Codex
Desktop plugin 快取也能正確註冊 bundled skill。

安裝後請開啟新的 Codex 對話，再呼叫 `$academic-research-suite-ads`，或直接描述符合
內建 workflow 的學術研究任務。
...
使用 `/skills` 驗證：您應該會看到一個 ARS-Codex-ADS 項目，即
`academic-research-suite-ads` 或 `ARS-Codex-ADS`。您**不應該**看到來自本套件的獨立 `academic-paper`、
...
使用 `$academic-research-suite-ads`（單數形式）明確呼叫套件，然後描述研究任務，
...
| Workflow | 適用情境 | 範例提示 |
|---|---|---|
| `deep-research` | 研究問題精煉、文獻回顧、系統性回顧、後設分析、事實查核 | `Use $academic-research-suite-ads to build a systematic review protocol for AI in higher education QA.` |
| `academic-paper` | 論文大綱、撰寫、摘要、修改、引用格式、AI 使用聲明 | `Use $academic-research-suite-ads to turn these notes into an IMRaD paper outline and drafting plan.` |
| `academic-paper-reviewer` | 稿件審閱、模擬同儕審查、編輯決策、重新審查 | `Use $academic-research-suite-ads to review this manuscript and produce a journal-style decision letter.` |
| `academic-pipeline` | 端到端的研究至論文 workflow，包含誠信閘門、審查、修改與最終檢查 | `Use $academic-research-suite-ads to run an end-to-end research-to-paper pipeline from topic to revised manuscript.` |
| `experiment-agent` | 程式碼實驗規劃、人類研究方案、統計詮釋、可重現性驗證 | `Use $academic-research-suite-ads to plan a code experiment and define reproducibility checks.` |

### Claude 風格別名

Claude Code v3.7 安裝 `/ars-*` 斜線指令。Codex 沒有相同的 plugin 指令註冊機制，
因此本套件在單一 `$academic-research-suite-ads` skill 內模擬指令意圖。可使用以下任一形式：
...
- `[features].codex_hooks is deprecated` — 方便時更新您的 Codex 設定；
  ARS-Codex-ADS 在正常使用下不需要 hooks。
- `hooks need review before they can run` — 如果您使用這些 hooks，
  請另外審查。ARS-Codex-ADS 將內嵌的 Claude hooks 視為可追溯性 metadata，
  不會要求它們。
...
|---|---|
| 單一可安裝 plugin | 原生 Codex plugin `ars-codex-ads`，內含單一 `academic-research-suite-ads` skill |
...
| Plugin marketplace 更新 | 執行 `codex plugin marketplace upgrade ars-codex-ads` 後重新加入 `ars-codex-ads@ars-codex-ads`；直接安裝的 skill 仍以重新安裝或 pull 更新 |
...
明確同意，ARS-Codex-ADS 將回退至單一執行時期審查，並應報告跨模型驗證不可用。

## 支持與贊助

如果 ARS-Codex-ADS 對您的研究 workflow 有所幫助，您可以透過
