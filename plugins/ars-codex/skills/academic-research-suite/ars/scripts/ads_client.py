...
class AdsUnavailable(Exception):
    """ADS API degraded; callers record the resolver as unreachable."""
...
class AdsClient:
    ...
    def __init__(self) -> None:
        ...
        self._user_agent = "ARS-v3.16-ADS"
            ...
            try:
                with urllib.request.urlopen(req, timeout=30) as resp:
                    ...
                    if not isinstance(data, dict):
                        raise AdsUnavailable("ADS response root is not an object")
                    response = data.get("response")
                    if not isinstance(response, dict):
                        raise AdsUnavailable("ADS response envelope is malformed")
                    docs = response.get("docs")
                    if not isinstance(docs, list) or not all(
                        isinstance(doc, dict) for doc in docs
                    ):
                        raise AdsUnavailable("ADS response docs is not a document list")
                    return docs
